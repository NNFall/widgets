...
## Прозрачная визуальная проверка и восстановление интерфейса

Финальный визуальный цикл состоит из независимых участников:

1. три Gemini-критика параллельно проверяют один и тот же набор desktop/mobile
   снимков;
2. отдельный Gemini-судья семантически объединяет замечания, даже если критики
   описали одну проблему разными словами;
3. на исправление передаются только проблемы, подтверждённые минимум двумя
   разными критиками;
4. после изменения отдельный Gemini-проверяющий сопоставляет diff артефакта с
   каждым замечанием судьи;
5. окончательное подтверждение всё равно даёт новый браузерный прогон и новый
   состав критиков.

В SSE-журнале для пользователя видны события `visual_critic.completed`,
`visual_judge.completed`, `visual_repair.completed` и
`visual_repair.verifier_completed`. Они содержат полный текст замечаний,
объединённый вердикт, понятное описание внесённых изменений и результат
независимой проверки кода.

Лимиты разделены по типам работы и больше не расходуют один общий счётчик:

- до 6 попыток получить браузерные доказательства;
- до 4 исправлений browser gate;
- до 4 исправлений детерминированной валидации;
- до 8 визуальных исправлений по умолчанию, максимум 10 в одном запуске;
- одна и та же семантическая проблема исправляется не более трёх раундов,
  независимо от формулировки критиков.

Последний технически отображаемый артефакт хранится как draft. Если финальная
проверка завершилась ошибкой, preview продолжает показывать этот draft с
пометкой `требует repair`; чат и публикация остаются доступны только для
проверенного артефакта.

Builder сохраняет активный run ID в namespaced-ключе
`kaigo.builder.activeRun.v1`. После перезагрузки страницы интерфейс заново
получает snapshot, восстанавливает форму, preview и всю историю SSE-событий.
Ключ удаляется автоматически только если сервер отвечает, что такой запуск
