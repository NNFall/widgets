...
| ЮKassa и тарифы | Sandbox checkout доказан | Реальный test-mode checkout, 1 990 ₽, HTTPS redirect, metadata verification и idempotency replay; 123 billing tests | Ручная тестовая оплата картой нужна для доказательства `succeeded`, сохранения метода и автопродления |
