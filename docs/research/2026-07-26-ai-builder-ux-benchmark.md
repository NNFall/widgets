...
После трёх серий визуальных референсов уточнён баланс между простотой и
насыщенностью:

- полноценный header с логотипом, навигацией и входом;
- сильный слоган и короткое объяснение продукта;
- один крупный composer как главное действие;
- composer принимает ссылку либо обычное описание;
- рядом или ниже — ровно одно компактное визуальное доказательство: короткий
  диалог, launcher, три тонких шага либо миниатюрное превращение сайта;
- преимущества, role chips, метрики и подробные примеры начинаются ниже hero;
- плотность не должна превращать hero в dashboard, плакат или витрину карточек.

Первый экран одновременно объясняет Kaigo и позволяет сразу воспользоваться
им. Различаться могут палитра, типографика, фон, расположение preview и motion,
но composer должен оставаться самым очевидным действием.

Первая, намеренно разреженная серия сохранена в
`docs/research/assets/2026-07-26-kaigo-landing-references/`. Вторая, слишком
насыщенная серия сохранена в
`docs/research/assets/2026-07-26-kaigo-landing-references-rich/`. Третья,
сбалансированная серия сохранена в
`docs/research/assets/2026-07-26-kaigo-landing-references-balanced/` и считается
текущим направлением.

Четвёртая серия уточнила не только плотность, но и путь взгляда. Три маленьких
шага `ссылка → анализ → AI готов` соединяются одной линией и приводят к composer.
Светлые и тёмно-синие варианты сохранены в
`docs/research/assets/2026-07-26-kaigo-landing-references-guided-flow/`. Эта
композиционная схема заменяет крупные preview и наборы разрозненных карточек.

Пятая серия вернула первому экрану визуальную выразительность, но сохранила одну
историю и один CTA. Крупная сцена справа теперь показывает трансформацию сайта,
пространственные порталы, AI-ядро либо генеративный объект. Результаты сохранены
в `docs/research/assets/2026-07-26-kaigo-landing-references-narrative/`.

Главный вывод: карточки могут быть крупными и содержательными. Проблема
возникает не из-за их размера, а когда они одинаковы, содержат по два слова и не
образуют общую сцену. Наиболее продуктово точная метафора — показать один и тот
же сайт до и после создания AI-виджета.

Шестая серия уточнила выбранные направления и усилила B2B-позиционирование. Во
всех вариантах результат называется AI-виджетом, а оффер обращается к бизнесу.
Для тёмных экранов использованы воспроизводимые палитры с HEX-кодами, найденные
по материалам Figma и открытым каталогам сочетаний. Серия сохранена в
`docs/research/assets/2026-07-26-kaigo-landing-references-business-refined/`.

Основным кандидатом считается структурированное «до/после»: один сайт сначала
показывается без виджета, проходит через анализ содержания и стиля, затем
возвращается с открытым AI-диалогом. Такой сценарий одновременно объясняет
продукт и даёт естественную основу для последовательной анимации.

Седьмая серия проверила эту гипотезу не отдельными статичными hero, а пятью
парами ключевых кадров `before → after`. Тёмная насыщенная сцена заменена
светлой: оффер и URL-composer остаются неподвижными слева, а справа один
условный сайт проходит сканирование. После волны появляются только компактный
чат и три выгоды. Варианты сохранены в
`docs/research/assets/2026-07-26-kaigo-landing-animation-pairs-light/`.

Такой формат ближе к будущему интерфейсу, потому что кадры задают не только
композицию, но и инварианты motion: что остаётся на месте, что проявляется и в
каком порядке пользователь получает доказательство результата.

Восьмая серия проверила более содержательное финальное состояние. Простые
подписи заменены тремя карточками с номером, иконкой, заголовком и пояснением.
Карточки появляются не как отдельный статичный блок, а продолжают траекторию
сканирования. Сам чат получает акцентную рамку и отдельный импульс, поэтому
понятно, какой именно продукт появился на сайте. Серия сохранена в
`docs/research/assets/2026-07-26-kaigo-landing-animation-cards-refined/`.

Риск решения: визуальное proof и дополнительные преимущества могут начать
конкурировать с composer. При сборке реального интерфейса это нужно проверять
по времени нахождения главного действия и первому клику пользователя.
