...
## Три конструкции первого экрана

### A. URL-first playground — рекомендуется

В центре находится крупный URL-first composer. Под ним — короткое пожелание и
сценарии. В нижней или правой части первого экрана — живая демонстрация:
обычный сайт получает индивидуальный launcher, тот раскрывается, посетитель
задаёт вопрос и получает ответ.

Плюсы: сразу объясняет механику и даёт совершить действие.

Риск: демонстрация не должна конкурировать с формой.

### B. Split proof

Слева обещание и URL-ввод, справа интерактивный before/after: «сайт без Kaigo» и
«тот же сайт с AI-сотрудником».

Плюсы: самое быстрое доказательство отличия.

Риск: на небольшом ноутбуке первый экран может стать тесным.

### C. Product story

Сначала короткий эмоциональный hero, затем последовательность «вставьте сайт →
посмотрите анализ → получите сотрудника» и только после неё ввод.

Плюсы: легче построить выразительную презентацию.

Риск: слабее конвертирует готового попробовать пользователя и повторяет обычный
SaaS-лендинг.

Рекомендуется гибрид A+B: форма остаётся главным объектом, а компактный
before/after служит доказательством, не отдельным длинным рассказом.
