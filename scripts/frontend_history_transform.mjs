...
const allowedViews = new Set(['landing', 'studio', 'tour']);
...
function renderViewWrapper(version, title, view) {
  if (!/^v[1-9][0-9]*$/.test(version)) throw new Error(`invalid archive version: ${version}`);
  if (!allowedViews.has(view) || view === 'landing') throw new Error(`invalid wrapped view: ${view}`);
  const safeTitle = escapeHtml(title);
  const viewLabel = view === 'studio' ? 'Studio' : 'Product tour';
  return `<!doctype html>
<html lang="ru">
  <head>
    <meta charset="UTF-8" />
...
function renderViewLink(version, view) {
  ...
  return `<a href="/frontend/${version}/${suffix}">${labels[view]}<span aria-hidden="true">↗</span></a>`;
}
...
async function transformDist(distDirectory, version) {
